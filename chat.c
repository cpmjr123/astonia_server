...
static void rec_chat(unsigned short channel,char *text) {
    ...
    if (channel==0) {
        ...
        if (channel==7 || channel==12 || channel==13) { cnr=atoi(text+11); step=14; } else if (channel==8) { anr=atoi(text+11); step=14; } else if (channel==9) { anr=atoi(text+11); mnr=atoi(text+14); step=17; } else step=10;
    ...
    if (!connected) {
        ...
        if (state==1) {
            ...
            if (FD_ISSET(sock,&fd_err)) {
                ...
                if (getsockopt(sock,SOL_SOCKET,SO_ERROR,&sol,&sollen)) { close(sock); delay=100; state=0; } else if (sol) { close(sock); delay=100; state=0; } else {
                    connected=1;
