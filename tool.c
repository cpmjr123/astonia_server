...
int can_attack(int cn,int co) {
    ...
    if ((ch[cn].flags&CF_PLAYER) && (ch[co].flags&CF_PLAYER)) {
        ...
        for (n=0; n<MAXHATE; n++) if (ppd->hate[n]==ch[co].ID) return 1;
...
int is_hate_empty(int cn) {
    ...
    for (n=0; n<MAXHATE; n++) if (ppd->hate[n]) return 0;
...
void add_hate(int cn,int co) {
    ...
    for (n=0; n<MAXHATE-1; n++) if (ppd->hate[n]==ch[co].ID) break;
...
void set_legacy_item_lvl(int in,int cn) {
    if (it[in].flags&(IF_WNHEAD|IF_WNNECK|IF_WNBODY|IF_WNARMS|IF_WNBELT|IF_WNLEGS|IF_WNFEET|IF_WNLHAND|IF_WNRHAND|IF_WNCLOAK|IF_WNLRING|IF_WNRRING|IF_WNTWOHANDED)) set_item_requirements_sub(in,ch[cn].level);
}
